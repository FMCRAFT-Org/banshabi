...
public class BanCommand implements CommandExecutor {
    ...
    public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
        if (!sender.hasPermission("banshabi.ban")) {
        ...
        if (filteredArgs.length < 3) {
            sender.sendMessage(BanShabi.getInstance().getPrefix() + 
                             BanShabi.getInstance().colorize(BanShabi.getInstance().getMessage("ban.invalid_time")));
            return true;
        }
        ...
        if (reasonStr.isEmpty()) {
            sender.sendMessage(BanShabi.getInstance().getPrefix() + 
                             BanShabi.getInstance().colorize(BanShabi.getInstance().getMessage("ban.no_reason")));
            return true;
        }

        PunishmentManager punishmentManager = BanShabi.getInstance().getPunishmentManager();
        
        if (punishmentManager.isBanned(playerName)) {
            sender.sendMessage(BanShabi.getInstance().getPrefix() + 
                             BanShabi.getInstance().colorize(BanShabi.getInstance().getMessage("ban.already_banned", 
                             "player", playerName)));
            return true;
        }
        ...
        Punishment punishment = punishmentManager.addPunishment(playerName, playerUuid, ipAddress, PunishmentType.BAN, 
                                                                 reasonStr, duration, operator);

        String timeDisplay = duration <= 0 ? "永久" : PunishmentManager.formatDuration(duration);
        
        sender.sendMessage(BanShabi.getInstance().getPrefix() + 
                         BanShabi.getInstance().colorize(BanShabi.getInstance().getMessage("ban.success", 
                         "player", playerName)));
