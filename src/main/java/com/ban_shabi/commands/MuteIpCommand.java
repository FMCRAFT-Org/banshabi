...
public class MuteIpCommand implements CommandExecutor {
    ...
    public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
        if (!sender.hasPermission("banshabi.muteip")) {
        ...
        if (filteredArgs.length < 3) {
            sender.sendMessage(BanShabi.getInstance().getPrefix() + 
                             BanShabi.getInstance().colorize(BanShabi.getInstance().getMessage("muteip.invalid_time")));
            return true;
        }
        ...
        if (reasonStr.isEmpty()) {
            sender.sendMessage(BanShabi.getInstance().getPrefix() + 
                             BanShabi.getInstance().colorize(BanShabi.getInstance().getMessage("muteip.no_reason")));
            return true;
        }

        PunishmentManager punishmentManager = BanShabi.getInstance().getPunishmentManager();
        
        if (punishmentManager.isMutedByIp(ipAddress)) {
            sender.sendMessage(BanShabi.getInstance().getPrefix() + 
                             BanShabi.getInstance().colorize(BanShabi.getInstance().getMessage("muteip.already_muted", 
                             "ip", ipAddress)));
            return true;
        }
        ...
        Punishment punishment = punishmentManager.addPunishment("IP:" + ipAddress, null, ipAddress, PunishmentType.MUTE, 
                                                                 reasonStr, duration, operator);

        String timeDisplay = duration <= 0 ? "永久" : PunishmentManager.formatDuration(duration);
        
        sender.sendMessage(BanShabi.getInstance().getPrefix() + 
                         BanShabi.getInstance().colorize(BanShabi.getInstance().getMessage("muteip.success", 
                         "ip", ipAddress)));
            ...
            if (playerIp != null && playerIp.equals(ipAddress)) {
                ...
                if (duration <= 0) {
                    muteMessage = BanShabi.getInstance().colorize(
                        BanShabi.getInstance().getMessage("muteip.muted_message_permanent",
                    ...
                    muteMessage = BanShabi.getInstance().colorize(
                        BanShabi.getInstance().getMessage("muteip.muted_message",
